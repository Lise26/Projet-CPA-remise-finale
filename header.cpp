...
void header::header_name(ifstream& headerFile, vector<int> offMax, vector<int> headerOffset){ // Init values of parameters by reading the header file
	...
	string n(size,0);
	charToString(n, name, size);
	names[indice] = n;
}
}
//ENLEVER ÇA
void header::charToString(string& s, vector<char> tab, int size){		//Convert from vector<char> to string
...
string const& header::getNames(int i){
	return names[i];
}
